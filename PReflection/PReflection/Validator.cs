		...
		public static  ErrorInfo[] Validate(object obj){
			...
			foreach (FieldInfo field in fields)
				if(field.IsDefined(typeof(ValidationAttribute),true)){
					ValidationAttribute validationAttribute = (ValidationAttribute)Attribute.GetCustomAttribute (type, typeof(ValidationAttribute));
					object value = field.GetValue(obj);
					string message = validationAttribute.Validate ();
					if(message != null){
